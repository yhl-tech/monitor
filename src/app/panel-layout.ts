...
import { IntelAnalystChat } from "@/components/IntelAnalyst"
import { IntelSituationOverlay } from "@/components/IntelSituation"
...
export class PanelLayoutManager implements AppModule {
  ...
  private _intelAnalystChat: IntelAnalystChat | null = null
  private _intelSituationOverlay: IntelSituationOverlay | null = null
  ...
  destroy(): void {
    ...
    this._intelAnalystChat?.destroy()
    this._intelAnalystChat = null
    this._intelSituationOverlay = null
  ...
  renderLayout(): void {
    this.ctx.container.innerHTML = `
      ...
      <div class="header">
        <div class="header-left">
          ...
          <button type="button" class="intel-analyst-header-toggle" id="intelAnalystHeaderToggle"
            aria-expanded="true"
            aria-controls="intelAnalystSidebar"
            title="${escapeHtml(t("intelAnalyst.headerToggleTitle"))}"
            aria-label="${escapeHtml(t("intelAnalyst.headerToggleTitle"))}">
          ...
          <div class="variant-switcher">${(() => {
            ...
            const vTarget = (v: string) =>
              !local && SITE_VARIANT !== v && inIframe
                ...
                : ""
            return `
            ${SITE_VARIANT !== "full" ? `<a href="${vHref("full", "https://worldmonitor.app")}"
            ...
            </a><span class="variant-divider"></span>` : ""}
            ${SITE_VARIANT !== "tech" ? `<a href="${vHref("tech", "https://tech.worldmonitor.app")}"
               class="variant-option"
               data-variant="tech"
               ${vTarget("tech")}
               title="${t("header.tech")}">
            ...
            ${SITE_VARIANT !== "finance" ? `<a href="${vHref("finance", "https://finance.worldmonitor.app")}"
               class="variant-option"
            ...
            ${SITE_VARIANT !== "commodity" ? `<a href="${vHref("commodity", "https://commodity.worldmonitor.app")}"
               class="variant-option"
               data-variant="commodity"
               ${vTarget("commodity")}
               title="${t("header.commodity")}">
      ...
      <div class="main-content" id="mainContent">
        <div id="intelAnalystSidebar" class="intel-analyst-sidebar"></div>
        <div class="intel-resize-handle" id="intelResizeHandle"></div>
        <div class="intel-hub-main">
    ...
    if (mapContainerEl) {
      this._intelSituationOverlay = new IntelSituationOverlay(mapContainerEl)
      this._intelSituationOverlay.setMapContext({
        fitCountry: (code: string) => {
          this.ctx.map?.fitCountry(code)
        },
        flyToCity: (lat: number, lon: number, zoom?: number, duration?: number) => {
          this.ctx.map?.flyToCity(lat, lon, zoom, duration)
        },
        forceSwitchToFlatMode: () => {
          return this.ctx.map?.forceSwitchToFlatMode() ?? null
        },
        restoreGlobeModeIfNeeded: (wasGlobeMode: boolean, prevCenter?: { lat: number; lon: number } | null) => {
          this.ctx.map?.restoreGlobeModeIfNeeded(wasGlobeMode, prevCenter)
        },
        ensureGlobeMode: () => {
          const map = this.ctx.map
          if (!map || map.isGlobeMode()) return false
          map.switchToGlobe()
          return true
        },
        setAutoRotate: (enabled: boolean) => {
          this.ctx.map?.setAutoRotate(enabled)
        },
      })
    }
    ...
    if (sidebarEl) {
      this._intelAnalystChat = new IntelAnalystChat(sidebarEl, {
        ...
        onVisibilityChange: (visible) => {
          const btn = document.getElementById("intelAnalystHeaderToggle")
          if (!btn) return
          btn.classList.toggle(
            "intel-analyst-header-toggle--collapsed",
            !visible,
          )
          btn.setAttribute("aria-expanded", String(visible))
        },
        // Called immediately when user submits — shows overlay before results arrive
        onSituationSubmit: (targetName) => {
          this._intelSituationOverlay?.showPending(targetName, "CN")
        },
        // Called when API returns — overlay flies to city and updates panels
        onSituationReady: (
          taskId,
          taskName,
          taskSummary,
          ...
          this._intelSituationOverlay?.updateWithResult(
            taskId,
            taskName,
            taskSummary,
            countryCode,
        ...
        onNewChat: () => {
          this._intelSituationOverlay?.clear()
        },
      })
      document
        .getElementById("intelAnalystHeaderToggle")
        ?.addEventListener("click", () => {
          this._intelAnalystChat?.toggleIntelPanel()
        })
    }
